...
The message player now renders a lightweight custom facade before YouTube loads. It uses the verified thumbnail as the photographic field and tightly isolates the tree from the approved local Living Message SVG, rendering it white inside a restrained deep-blue-to-purple gradient circle surrounded by a 16-second rotating “Watch message” ring. The complete facade is one keyboard-accessible play button; activation replaces it with the privacy-enhanced `youtube-nocookie.com` iframe and requests autoplay. Reduced-motion styling leaves the ring static.
