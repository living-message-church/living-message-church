...
Source review confirms two featured, current-size cards for Brian and Allison followed by a more compact 4-column desktop grid for every other team member. The final two Deacon cards center as a pair rather than hanging against one edge. The directory becomes a 2-column tablet composition and one bounded portrait card per row below 48rem. Cards use a stable 7:10 frame, natural document-height copy, quiet spacing, no large shadow, and only a restrained crop transition that is suppressed by the global reduced-motion rule. The page has one `h1`, tier `h2` headings, person `h3` headings, indexable metadata, and no interactive card behavior requiring a keyboard target.
