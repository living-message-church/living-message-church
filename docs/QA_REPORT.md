...
The player is a responsive official `youtube-nocookie.com` iframe with fullscreen and picture-in-picture permission, no forced autoplay, a descriptive title, and `LIVE NOW`, `Next live service`, or `Live channel` labeling. When no active/future broadcast is resolved, the same 16:9 window becomes a branded motion thumbnail with the approved navy-to-purple color, softly drifting wave lines, the white Living Message tree, and a rotating next-broadcast ring. The former lower-left offline caption and channel link are removed. It replaces YouTube's “video unavailable” screen without introducing a recorded-message fallback. All decorative motion is neutralized by the global reduced-motion rule. Cinema now repositions the existing mounted iframe over a dark moving veil instead of creating a second player, so entering, exiting, or using YouTube fullscreen preserves playback position. Escape, the close control, or clicking outside the player exits Cinema.

The Church Online schedule remains present independently of Planning Center availability. It calculates the next Sunday date in the Eastern time zone and displays the user-verified recurring broadcast time of 10:45 AM EST in a compact, non-interactive lower-left video overlay. Moving it out of the copy column restores that column's prior hierarchy; the overlay contains no channel link. The hero's past-message wording links directly to `/messages` rather than describing the provider.

The Message Archive keeps the newest message featured at the top with the rotating signature play treatment, inline playback, and Cinema. Archive cards use a quieter transparent circular play control and play directly inside their own frames instead of replacing the featured player or scrolling upward. Each card also exposes Cinema mode, and the library coordinates playback so starting another inline message stops the previously active inline player. Cinema repositions that same mounted iframe, preserving playback while entering and exiting; its deep navy-to-purple veil carries very slow, faint radial-light movement, while reduced-motion visitors receive the same static gradient. The homepage message presentation remains unchanged.
