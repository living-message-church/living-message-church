...
Status: foundation implemented; generation and approval blocked by admin authentication.
...
The application does not yet have admin authentication. Consequently, no scan-queue, generation, selection, approval, or rejection action is exposed over HTTP. The server workflow helpers require an authenticated administrative actor, but there is intentionally no caller until authentication, authorization, CSRF protection, and audit identity are approved.

The visible “Scan eligible Calendar events” control is disabled for the same reason. Its read-only preview already runs the Planning Center GET → canonicalization → eligibility portion; persistence of missing-artwork jobs is withheld until the authenticated server-action boundary exists.
