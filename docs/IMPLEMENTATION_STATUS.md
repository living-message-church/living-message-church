...
- `/online-church` is now the canonical Church Online destination: its existing media window always targets the verified channel's official privacy-enhanced live-stream embed instead of substituting a completed sermon. Server-resolved Live/Upcoming metadata informs the label and schedule only; API failure retains the live player and channel link. Past-message actions route to `/messages`; `/messages/live` redirects permanently to the production-authoritative slug.
...
| YouTube API failure | The page retains the 16:9 verified-channel live embed and channel fallback even when status metadata cannot refresh; it does not substitute an old completed message. |
