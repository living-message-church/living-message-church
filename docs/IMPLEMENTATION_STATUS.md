...
- `/messages` now provides an hourly revalidated newest-first YouTube feed, a latest-message featured player, publication dates, evidence-backed category filters, keyword search, responsive archive cards with in-card playback and Cinema mode, coordinated single inline playback, and truthful local fallback.
